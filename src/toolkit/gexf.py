...
class GexfGen(object):
    ...
    def getNodeStrLst(self, nodeIdx):
        nodeStrLst = []
        if (len(self.nodes[self.idToNode[nodeIdx]]) == 0): return [
            '<node id="{0}" label="{1}" />'.format(nodeIdx, self.idToNode[nodeIdx])]
    ...
    def getEdgeStr(self, edgeIdx):
        ...
        return '<edge id="{0}" source="{1}" target="{2}" weight="{3}"/>'.format(edgeIdx, sourceNodeId, targetNodeId,
                                                                                weight)
    ...
    def getGraphStr(self):
        ...
        for i in range(len(self.attToId)): attStrLst.append(
            self.indent + '<attribute id="{0}" title="{1}" type="{2}"/>'.format(i, self.idToAtt[i],
                                                                                self.attToType[self.idToAtt[i]]))
